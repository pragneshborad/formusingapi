...
if(! defined('MAX_FILE_SIZE') )				define ('MAX_FILE_SIZE', 10485760000000);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
...
if(! defined('MAX_WIDTH') ) 			define ('MAX_WIDTH', 2500);									// Maximum image width
if(! defined('MAX_HEIGHT') ) 			define ('MAX_HEIGHT', 2500);								// Maximum image height
...
class timthumb {
	...
	protected function param($property, $default = ''){
		if (isset ($_GET[$property])) {
			 ...
			 // $string = urlencode($_GET[$property]);
			 // $string = str_replace("+", "%2B",$string);
			 // $string = urldecode($string);

			if($property == "src"){
				// $explode = explode("=", $_SERVER['QUERY_STRING']);
				// $str_replace = str_replace("&h", "", $explode[1]);
				// $str_replace = str_replace("&w", "", $str_replace);
				// $str_replace = str_replace("%20"," ", $str_replace);
				// $str_replace = str_replace("%27","'", $str_replace);
				// return $str_replace;

				$this->localImage = $this->getLocalImagePath($_GET[$property]);
				if(! $this->localImage){
					$explode = explode("=", $_SERVER['QUERY_STRING']);
					$str_replace = str_replace("&h", "", $explode[1]);
					$str_replace = str_replace("&w", "", $str_replace);
					$str_replace = str_replace("%20"," ", $str_replace);
					$str_replace = str_replace("%27","'", $str_replace);
					$str_replace = str_replace("%E2%80%93", "–", $str_replace);
					$str_replace = str_replace("%C3%A8", "è", $str_replace);
					$str_replace = str_replace("%C3%A4", "ä", $str_replace);
					$str_replace = str_replace("%C3%BC", "ü", $str_replace);
					$str_replace = str_replace("%C2%A0", " ", $str_replace);
